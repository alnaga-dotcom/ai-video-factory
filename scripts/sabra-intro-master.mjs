...
function run(command, args) { return new Promise((ok, fail) => { const child = spawn(command, args, { stdio: "inherit", shell: false, windowsHide: true }); child.on("error", fail); child.on("exit", code => code === 0 ? ok() : fail(new Error(`${command} exited ${code}`))); }); }
...
const filter = "[1:v]scale=260:-1[logo];[0:v][logo]overlay=W-w-32:32:enable='between(t,5.5,8)'[v];[2:a]atrim=0:8,asetpts=PTS-STARTPTS,volume=0.82[music];[3:a]atrim=0:2.2,asetpts=PTS-STARTPTS,adelay=3600:all=1,volume=0.58[laugh];[music][laugh]amix=inputs=2:duration=first:normalize=0,alimiter=limit=0.95[a]";
