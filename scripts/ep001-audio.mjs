...
const out = resolve("staging/sabra-world/episodes/EP001/audio");
await mkdir(out, { recursive: true });

const segments = [
  ["01-introduction", "قبل ما نبدأ... خلّيني أعرّفكم بنفسي. أنا صابرة... بنت الصعيد."],
  ["02-origin", "اتولدت في الصعيد، وعشت هناك وسط أهلي وناسي، واتربيت على الأصول، وعلى إن البيت والعيلة هما السند."],
  ["03-marriage", "ولما بقى عندي تمنتاشر سنة، اتجوزت صابر، وجيت معاه نعيش في قرى جنوب الجيزة."],
  ["04-life", "شربت من مية النيل، وأكلت من خير بلدنا. وعندي الأرض، والبيت، والعيلة الحلوة."],
  ["05-years", "عشت عمري وسط أهلي وناسي، وشُفت أيام حلوة... وأيام تقول: يا ساتر يا رب! اتجوزت، وخلفت، وربّيت... وكبروا العيال، بس الحكايات ما خلصتش."],
  ["06-promise", "أصل كل بيت له حكاية... وأنا عندي حكايات تكفّي بلد بحالها! معايا هتسمعوا حكايات أصيلة، وهنتعلم سوا أكلات حلوة من خير بلدنا."],
  ["07-slogan", "صابرة... حكايات من قلب البيت المصري. حكاية، وضحكة، وأكلة... وكل يوم حكاية جديدة."],
  ["08-next", "تابعوني في الحلقة اللي جاية... أصل الحكاية لسه بتبدأ!"],
];
...
for (const [id, text] of segments) {
  ...
  await ffmpeg(["-y", "-i", raw, "-filter:a", "atempo=1.08,atempo=1.08,volume=1.15", master]);
  processed.push(master);
  console.log(`  SAVED ${master}`);
}

const concatFile = resolve(out, "concat.txt");
await writeFile(concatFile, processed.map(p => `file '${p.replaceAll("'", "'\\''")}'`).join("\n") + "\n", "utf8");
const full = resolve(out, "EP001-sabra-voice-review.wav");
await ffmpeg(["-y", "-f", "concat", "-safe", "0", "-i", concatFile, "-c:a", "pcm_s16le", full]);
await writeFile(resolve(out, "script.txt"), segments.map(([id, text]) => `[${id}]\n${text}`).join("\n\n") + "\n", "utf8");
console.log("EP001 AUDIO PASS — QC REQUIRED BEFORE VIDEO GENERATION");
console.log(`REVIEW ${full}`);
console.log("If one segment has a pronunciation/performance issue, fix/regenerate that segment rather than the whole episode.");
